...
class Generator(nn.Module):
    ...
    def __init__(self):
        ...
        encoder_layer = TransformerEncoderLayer(TN_HIDDEN_DIM, TN_NHEADS, TN_DIM_FEEDFORWARD, TN_DROPOUT, "relu", True)
        ...
        decoder_layer = TransformerDecoderLayer(TN_HIDDEN_DIM, TN_NHEADS, TN_DIM_FEEDFORWARD, TN_DROPOUT, "relu", True)
        decoder_norm = nn.LayerNorm(TN_HIDDEN_DIM)
        self.decoder = TransformerDecoder(decoder_layer, TN_DEC_LAYERS, decoder_norm, return_intermediate=True)
...
class TRGAN(nn.Module):
    ...
    def _generate_page(self, ST, SLEN):
        ...
        for idx, st in enumerate((sdata_)):
            ...
            word_t.append((st[0,0,:,:int(SLEN.cpu().numpy()[0][idx])].cpu().numpy()+1)/2)
